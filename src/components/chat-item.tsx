...
import { useVoiceFile } from '@/hooks/use-voice-file'; 
...
const ChatItem: FC<ChatItemProps> = ({
  ...
  const { publicUrl: chatFileUrl, fileType } = useChatFile(fileUrl!, null);
  const { publicUrl: voiceFileUrl } = useVoiceFile(fileUrl!, null); 
  ...
  const isVoice = fileUrl?.startsWith('voice/') && voiceFileUrl; 
  ...
  const FilePreview = () => (
    <>
      ...
      {isVoice && (
        <audio
          controls
          className='mt-2 w-full border rounded-md p-2 bg-secondary'
        >
